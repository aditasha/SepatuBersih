...
class OrderDriverRepositoryImpl @Inject constructor(
    private val firebaseDatabase: FirebaseDatabase,
    private val firebaseAuth: FirebaseAuth,
    private val firebaseStorage: FirebaseStorage,
    private val serverTime: ServerTime
    ) : OrderDriverRepository {

    override val currentUser: FirebaseUser?
        get() = firebaseAuth.currentUser

    override suspend fun updateOrderStatus(sbOrder: SbOrder, node: String, status: String): Result<SbOrder> {
        return try {
            Result.Loading
            val ongoingItemRef = firebaseDatabase.reference.child(RealtimeDatabaseConstants.USER_ORDER_ITEM)
            ...
            val orderDetailRef = firebaseDatabase.reference.child(RealtimeDatabaseConstants.ORDER_DETAIL)
                .child(sbOrder.key!!)

            val allOrderRef = firebaseDatabase.reference.child(RealtimeDatabaseConstants.ALL_ORDER_ITEM)
                .child(sbOrder.key!!)

            val orderNodeRef = firebaseDatabase.reference.child(RealtimeDatabaseConstants.ORDER_NODE)
                .child(sbOrder.key!!)

            val ongoingItemPath = ongoingItemRef.toString().substring(ongoingItemRef.root.toString().length)
            val completeItemPath = completeItemRef.toString().substring(completeItemRef.root.toString().length)
            val orderNodePath = orderNodeRef.toString().substring(orderNodeRef.root.toString().length)
            ...
            if (node == RealtimeDatabaseConstants.COMPLETE) {
                ...
                sbOrder.apply {
                    sbOrderItem = SbOrderItem(
                        this.key, id, name, orderTimestamp, this.status, statusTimestamp, endTimestamp, price
                    )
                }
            ...
            //Ongoing
                ...
                if (status == RealtimeDatabaseConstants.PICKUP || status == RealtimeDatabaseConstants.DELIV) {
                    updates["driver"] = currentUser?.uid
                }
                else {
                    updates["driver"] = null
                }
